// NOTE - this code is ported from the Xbee-Arduino library.  Full copyright of the original code is below... 
...
XBee::XBee(): _response(XBeeResponse()) {
	...
	// NOTE - _serial not set here: must pass it in begin() or setSerial() instead
...
void XBee::begin(GMSerial &serial) {
	_serial = &serial;
}

//void XBee::setSerial(Stream &serial) {
void XBee::setSerial(GMSerial &serial) {
	_serial = &serial;
}
...
bool XBee::readPacket(int timeout) {
	...
	unsigned long start = DateTime::millis();

    while (int((DateTime::millis() - start)) < timeout) {
...
uint8_t XBeeWithCallbacks::waitForInternal(uint8_t apiId, void *response, uint16_t timeout, void *func, uintptr_t data, int16_t frameId) {
	...
	} while (DateTime::millis() - start < timeout);
